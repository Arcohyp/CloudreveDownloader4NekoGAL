# Cloudreve Downloader v3.0

**Cloudreve 分享链接一键下载工具**
...
# 直接传入链接
.\cloudreve-downloader.ps1 -ShareLink "https://pan.xxx.com/s/xxxxx"

# 指定下载目录
.\cloudreve-downloader.ps1 -ShareLink "https://pan.xxx.com/s/xxxxx" -OutputDir "D:\Downloads"

# 使用 32 线程加速
.\cloudreve-downloader.ps1 -ShareLink "https://pan.xxx.com/s/xxxxx" -Aria2Connections 32
```
...
https://pan.xxx.com/home?path=cloudreve%3A%2F%2Fxxxxx%40share
```
...
### Q: 下载速度很慢？
**A:** 
- 这是 Cloudreve 存储后端（如 Cloudflare R2）的限制，不是脚本问题
- 尝试增加线程数：在 `config.json` 中设置 `"defaultConnections": 32`
- 尝试不同时段下载（晚间可能较慢）
- 如果使用代理，检查代理速度
...
# 指定所有参数
.\cloudreve-downloader.ps1 `
    -ShareLink "https://pan.xxx.com/s/xxxxx" `
    -OutputDir "D:\MyDownloads" `
    -Aria2Connections 32
```
...
**Made with ❤️ for Cloudreve users**
